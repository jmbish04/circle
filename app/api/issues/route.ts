...
import { issues, users, statuses, priorities, projects } from '@/lib/db/schema';
import { eq } from 'drizzle-orm';

type DbBinding = Parameters<typeof getDb>[0];

type CloudflareRequestContext = NextRequest & {
   ctx?: {
      cloudflare?: {
         env?: {
            DB?: DbBinding;
...
function getCloudflareContext(request: NextRequest) {
   ...
   return (request as CloudflareRequestContext).ctx?.cloudflare?.env;
}
