...
export const projects = sqliteTable('projects', {
  ...
  healthId: text('health_id').notNull().references(() => health.id),
});
...
export const issueSubissues = sqliteTable('issue_subissues', {
  parentIssueId: text('parent_issue_id').notNull().references(() => issues.id),
  subissueId: text('subissue_id').notNull().references(() => issues.id),
});
